...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `obtenerRegistroYVehiculoAsociadoPorNumeroDeRegistro` (IN `numRegistro` INT(11) UNSIGNED)   SELECT * FROM v_registros_y_vehiculos_aociados WHERE numero_de_registro = numRegistro$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `obtenerUsuarioPorNombreUsuario` (IN `nombre` VARCHAR(255))   SELECT * FROM usuarios WHERE nombre_usuario = nombre$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `obtenerVehiculoPorPlacas` (IN `placasBuscadas` VARCHAR(255)) SELECT * FROM vehiculos WHERE placas = placasBuscadas$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `obtenerVehiculoPorSerie` (IN `serieBuscada` VARCHAR(255)) SELECT * FROM  vehiculos WHERE serie = serieBuscada$$
...
--
ALTER TABLE `registros_de_mantenimiento_de_vehiculo`
  ADD CONSTRAINT `registros_de_mantenimiento_de_vehiculo_ibfk_1` FOREIGN KEY (`placas_de_vehiculo`) REFERENCES `vehiculos` (`placas`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
